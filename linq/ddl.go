...
func ddlSetSync(model *Model) string {
	result := jdb.SQLDDL(`
	DROP TRIGGER IF EXISTS REDORDS_BEFORE_INSERT ON $1 CASCADE;
	CREATE TRIGGER REDORDS_BEFORE_INSERT
	BEFORE INSERT ON $1
	FOR EACH ROW
	EXECUTE PROCEDURE core.REDORDS_BEFORE_INSERT();

	DROP TRIGGER IF EXISTS REDORDS_BEFORE_UPDATE ON $1 CASCADE;
	CREATE TRIGGER REDORDS_BEFORE_UPDATE
	BEFORE UPDATE ON $1
	FOR EACH ROW
	EXECUTE PROCEDURE core.REDORDS_BEFORE_UPDATE();

	DROP TRIGGER IF EXISTS REDORDS_BEFORE_DELETE ON $1 CASCADE;
	CREATE TRIGGER REDORDS_BEFORE_DELETE
	BEFORE DELETE ON $1
	FOR EACH ROW
	EXECUTE PROCEDURE core.REDORDS_BEFORE_DELETE();
	
	DROP TRIGGER IF EXISTS REDORDS_AFTER_INSERT ON $1 CASCADE;
	CREATE TRIGGER REDORDS_AFTER_INSERT
	AFTER INSERT ON $1
	FOR EACH ROW
	EXECUTE PROCEDURE core.REDORDS_AFTER_INSERT();

	DROP TRIGGER IF EXISTS REDORDS_AFTER_UPDATE ON $1 CASCADE;
	CREATE TRIGGER REDORDS_AFTER_UPDATE
	AFTER UPDATE ON $1
	FOR EACH ROW
	EXECUTE PROCEDURE core.REDORDS_AFTER_UPDATE();

	DROP TRIGGER IF EXISTS REDORDS_AFTER_DELETE ON $1 CASCADE;
	CREATE TRIGGER REDORDS_AFTER_DELETE
	AFTER DELETE ON $1
	FOR EACH ROW
	EXECUTE PROCEDURE core.REDORDS_AFTER_DELETE();`, model.Table)
