...
| `request/`               | HTTP client utilities for outbound calls (GET, POST, PUT, DELETE, PATCH, OPTIONS with TLS support). All verbs funnel into `httpDo`; `Http`/`HttpCtx` and the `<Verb>WithTls` helpers pass `timeout=0`, while `Http[Ctx]WithTimeout` and `<Verb>WithTimeout(..., timeout, defaultValue)` run the call in a goroutine and, on expiry, return `newBody(defaultValue)` + `Status{Code: 408, Message: "timeout"}`. **Known caveats** (as of the uncommitted rework): the timeout only abandons the result — it does not cancel the in-flight request (no `context.WithTimeout`), so the goroutine/connection lives until the 120-minute client timeout; and the `multipart/form-data` branch writes to a nil `*bytes.Buffer` and panics, now inside a goroutine where callers/`Recoverer` cannot catch it |
